...
/**
 * Store central du deck Découverte — port de `deckStore` (mobile).
 *
 *  · Revenir sur l'onglet Découverte NE recharge RIEN : le deck vit ici, hors
 *    des écrans, et se consomme là où on l'avait laissé.
 *  · Quand la pile baisse, on recharge PAR AJOUT (pagination intelligente) —
 *    les profils déjà swipés étant exclus côté serveur, chaque appel ramène la
 *    suite, jamais un remplacement visible.
 *  · Les photos des prochains profils sont PRÉCHARGÉES par le navigateur dès
 *    l'arrivée des données : la première carte s'affiche instantanément.
 *  · Un like / pass / favori « consomme » le profil ici : tous les écrans
 *    restent synchronisés sans refetch.
 */
...
function prefetchAvatars(profiles: DiscoveryProfile[]) {
  ...
  // Le navigateur télécharge et met en cache — la 1re image d'abord.
...
interface DeckState {
  ...
  loadInitial: (
    key: string,
    fetcher: () => Promise<DiscoveryProfile[]>,
  ) => Promise<void>;
...
export const useDeckStore = create<DeckState>((set, get) => ({
  ...
  loadInitial: async (key, fetcher) => {
    ...
    if (s.filtersKey === key && (remaining > 0 || s.status === "loading"))
      return;
  ...
  refill: async (fetcher) => {
    ...
    try {
      ...
      const additions = fresh.filter(
        (p) => !known.has(p.id) && !consumed.has(p.id),
      );
      set((state) => ({
  ...
  consume: (id) =>
    set((s) => ({ consumedIds: new Set(s.consumedIds).add(id) })),
