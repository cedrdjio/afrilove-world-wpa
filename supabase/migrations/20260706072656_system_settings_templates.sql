...
-- maintenance_mode et feature flags sont lus par l'app (public) ;
-- les autres réglages restent réservés aux admins. Ici, lecture publique
-- des flags publics + lecture admin complète, écriture via RPC.
...
-- Lecture des réglages (tous, admin).
...
-- Modèles de messages : liste + upsert.
...
begin
  ...
  return (select coalesce(jsonb_agg(to_jsonb(t) order by t.channel, t.name), '[]'::jsonb)
