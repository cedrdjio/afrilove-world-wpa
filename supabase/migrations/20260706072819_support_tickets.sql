...
create table public.support_messages (
  ...
  is_internal boolean not null default false,   -- note interne (invisible du membre)
...
-- Le membre voit ses propres tickets (hors notes internes) ; l'équipe voit tout.
...
-- File des tickets : { total, counts, items }.
...
begin
  if public.admin_role_level() < 1 then
...
-- Détail d'un ticket + fil de messages (notes internes incluses).
...
begin
  ...
  values ((select auth.uid()), case when p_internal then 'ticket.note' else 'ticket.reply' end,
          'ticket', p_ticket_id::text, '{}'::jsonb);
end;
$$;
...
-- Mise à jour du ticket (statut, priorité, assignation).
