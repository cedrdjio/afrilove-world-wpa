...
-- ── A10 — gestion des membres du back-office ─────────────────────────
-- Liste des admins avec leur e-mail.
...
-- Ajout / mise à jour d'un membre back-office par e-mail (compte existant).
...
begin
  ...
  if v_uid is null then
    raise exception 'Aucun compte avec cet e-mail. La personne doit d''abord créer un compte.'
      using errcode = 'P0002';
  end if;
...
-- Activation / désactivation d'un membre back-office.
...
begin
  ...
  values ((select auth.uid()),
          case when p_active then 'admin.enable' else 'admin.disable' end,
          'admin', p_user_id::text, '{}'::jsonb);
end;
$$;
...
-- Révocation totale d'un accès back-office.
...
begin
  ...
  if p_user_id = (select auth.uid()) then
    raise exception 'cannot revoke your own access' using errcode = '42501';
  end if;
...
-- ── A11 — journal d'audit ────────────────────────────────────────────
-- Actions back-office filtrées : { total, items }.
...
-- Journal de connexions (auth) — sécurité.
...
begin
  ...
  return (
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'actor', ale.payload ->> 'actor_username',
      'ip', ale.ip_address,
      'created_at', ale.created_at
