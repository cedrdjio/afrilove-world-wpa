...
Deno.serve(async (req) => {
  ...
  if (!apiToken) {
    console.error(
      "[payment-initiate] CAMERPAY_API_TOKEN is not set (Vault or env)",
    );
  ...
  const invoiceId =
    `AFL-${user.id.slice(0, 8)}-${Date.now().toString(36)}`.toUpperCase();
  ...
  try {
    ...
    if (!camerpayRes.ok || !camerpayJson?.success || !camerpayJson?.pay_url) {
      console.error(
        "[payment-initiate] CamerPay rejected",
        camerpayRes.status,
        camerpayJson,
      );
      await admin
