...
function s3Endpoint(): string {
  ...
  const host = new URL(supabaseUrl).host.replace(
    ".supabase.co",
    ".storage.supabase.co",
  );
...
const CORS_HEADERS = {
  ...
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-kyc-part",
...
Deno.serve(async (req) => {
  ...
  const putResponse = await s3.fetch(
    `${s3Endpoint()}/${BUCKET}/${objectPath}`,
    {
      method: "PUT",
      body,
