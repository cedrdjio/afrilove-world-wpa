...
const CAMERPAY_BASE = (
  Deno.env.get("CAMERPAY_BASE_URL") ?? "https://camerpay.biz"
).replace(/\/$/, "");
...
async function settleByRecheck(
  ...
  try {
    const res = await fetch(
      `${CAMERPAY_BASE}/api/payment/${transactionUuid}/status`,
      {
        headers: {
  ...
  if (remoteStatus === "completed") {
    ...
    if (error) {
      const isTerminal = /PAYMENT_NOT_FOUND|AMOUNT_MISMATCH/.test(
        error.message,
      );
...
Deno.serve(async (req) => {
  ...
  if (hasSignedShape) {
    ...
    console.warn(
      "[payment-webhook] signature mismatch — falling back to recheck",
      {
        transactionUuid,
        invoiceId,
      },
  ...
  console.warn(
    "[payment-webhook] unusable payload",
    Object.keys(payload ?? {}),
  );
